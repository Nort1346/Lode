...
import { searchSeasonTorrents, toStreamEvent } from '#server/utils/torrents/torrent-search'
import { TORRENT_STREAM_KEEPALIVE_MS } from '#server/types/torrent-search'
import type { SeasonTorrentsOutcome, TorrentStreamEvent } from '#server/types/torrent-search'
...
export default defineEventHandler(async (event) => {
  ...
  let outcome: SeasonTorrentsOutcome
  try {
    outcome = await searchSeasonTorrents(showId, seasonNumber, locale, (e) => send(toStreamEvent(e)))
  } catch {
