...
function buildRows(ctx: StepContext): SummaryRow[] {
  ...
  const rows: SummaryRow[] = [{ label: 'Lode', value: 'http://localhost:5757' }]
  const qbit = selection.qbittorrent === 'local' ? 'http://localhost:8080' : urls.qbittorrent
  rows.push({ label: 'qBittorrent', value: qbit })
  const prowlarr = selection.prowlarr === 'local' ? 'http://localhost:9900' : urls.prowlarr
  rows.push({ label: 'Prowlarr', value: prowlarr })
  if (selection.mediaMode === 'local') {
    rows.push({ label: 'Jellyfin', value: 'http://localhost:8096' })
  } else if (selection.mediaMode === 'external') {
    rows.push({ label: 'Jellyfin', value: urls.jellyfin })
  } else {
    rows.push({ label: 'Jellyfin', value: '(disabled)' })
  }
  if (selection.flaresolverr) rows.push({ label: 'FlareSolverr', value: 'http://localhost:8191' })
  ...
  if (selection.dozzle) rows.push({ label: 'Dozzle', value: 'http://localhost:8082' })
  return rows
}
...
export async function showSummary(ctx: StepContext): Promise<void> {
  ...
  // Plain values: hyperlink() would break note()'s width calc and misalign the border.
  const rows = buildRows(ctx).map((row) => `  ${row.label.padEnd(18)} ${row.value}`)
  note(rows.join('\n'), 'Services')
